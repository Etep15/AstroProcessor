...
def load_run_manifest(run_root: str) -> dict[str, Any]:
    manifest = Path(run_root) / "run-manifest.json"
    if not manifest.is_file():
        raise OrchestrationError(
            f"Exact helper run manifest is missing: {manifest}. "
            "The helper contract uses run-manifest.json; do not discover another path."
        )
...
def command_self_test(args: argparse.Namespace) -> int:
    return emit({
        ...
        "run_manifest_exact_path_contract": True,
        "canonical_manifest_filename": "ghs-pass2-manifest.json",
        "run_and_canonical_manifest_names_distinct": True,
        "selection_publication_requires_no_path_discovery": True,
        "state_directory_compatibility": ".siril-ghs-stretch-pass2-v1.3.1",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Standalone GHS pass-2 v1.3.2 orchestration wrapper."
    )
