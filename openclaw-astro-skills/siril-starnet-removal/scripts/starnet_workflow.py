...
def execute_native_candidate(
    *,
    ...
    if repository_mask is None or repository_unscreen is None:
        raise NativeStarMaskError(
            "Expected exactly one starmask and one native unscreen layer; "
...
def _publish_selected(
    ...
    *,
    ...
    if existing_stable and not fresh_run:
        raise NativeStarMaskError(
            f"Canonical StarNet directory already exists: {paths['stable']}. "
            "Use --fresh-run to create a new result while preserving the "
            "existing directory intact."
        )
    ...
    staged_starless = publish / "SHO-starless-linear.fit"
    staged_mask = publish / "SHO-starmask.fit"
    staged_unscreen = publish / "SHO-stars-unscreen.fit"
    ...
    if preserved_existing is not None and preserved_existing.exists():
        raise NativeStarMaskError(
            f"Preservation destination already exists: {preserved_existing}"
        )
    ...
    manifest = {
        ...
        "publication_method": (
            "validate new result, preserve existing canonical directory, "
...
def run_project(
    ...
    if paths["stable"].exists() and not fresh_run:
        raise NativeStarMaskError(
            ...
            "This command did not reuse it. Rerun with --fresh-run to execute "
            "StarNet again and preserve the existing directory."
        )

    base.siril_version()
    sho_manifest, _sho_hash = base.load_sho_manifest(
        base.project_paths(workspace, project_name)
    )
    source_evidence = base.validate_input(
        base.project_paths(workspace, project_name),
        sho_manifest,
    )
    ...
    if selected is None:
        result = {
            ...
            "message": (
                "No StarNet starmask satisfied all semantic and "
                "diffuse-structure gates. Existing canonical output was not "
                "changed."
            ),
        }
...
def status_project(
    ...
    for key, path, expected_record in definitions:
        if not path.is_file():
            errors.append(f"Missing {key}: {path}")
            continue

        current_evidence = asdict(
            base.inspect_fits(
                path,
                expected_channels=3,
                require_float32=True,
        ...
        current_evidence["path"] = str(path)
        current_records[key] = current_evidence

        expected_hash = expected_record.get("sha256")
        if expected_hash and current_evidence["sha256"] != expected_hash:
    ...
    return {
        ...
        "manifest": str(paths["stable_manifest"]),
        "errors": errors,
        "linear_starless": current_records.get("linear_starless"),
        "starmask": current_records.get("starmask"),
        "unscreen_stars": current_records.get("unscreen_stars"),
...
def self_test(workspace: Path, timeout_seconds: int) -> dict[str, Any]:
    ...
    semantics = candidate["quality_assessment"]["starmask_semantics"]
    self_test_failures = []
    if candidate["status"] != "satisfactory":
        self_test_failures.append(
            "StarNet candidate did not pass its complete quality gate"
        )
    if semantics["finite_fraction"] != 1.0:
        self_test_failures.append("non-finite starmask")
    if semantics["negative_fraction"] != 0.0:
        self_test_failures.append("negative starmask values")
    if semantics["quantized_16bit_fraction"] < 0.995:
        self_test_failures.append("starmask is not 16-bit-derived")
    if semantics["nonzero_fraction"] < 0.001:
        self_test_failures.append("starmask is empty")
    ...
    if self_test_failures:
        raise NativeStarMaskError(
            f"Synthetic starmask semantics failed {self_test_failures}; preserved at {root}"
        )
    result = {
        "status": "success",
        "created_at": utc_now(),
        ...
        "tests": [
            ...
            "starmask 16-bit quantization",
            "starmask nonnegative sparse background",
            "linear starless inverse MTF output",
            "starmask not treated as additive difference",
            "near-zero starmask ratio stability",
            "linked and unlinked starmask previews",
            "unscreen stars preview",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate and validate the canonical StarNet starless, starmask and unscreen products."
    )
    ...
    run_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Execute StarNet again. If processing/starnet already exists, "
            "preserve it intact under the new run directory before publishing."
        ),
    )
