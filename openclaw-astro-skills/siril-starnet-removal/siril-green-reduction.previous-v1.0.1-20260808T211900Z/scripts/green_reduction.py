...
COMPATIBLE_RUN_HELPER_VERSIONS = frozenset({"1.0.1"})
COMPATIBLE_CANONICAL_HELPER_VERSIONS = frozenset({"1.0.1"})
...
def compact_review_plan(plan: dict[str, Any]) -> dict[str, Any]:
    payload = {"status": plan["status"], "action": plan["action"], "helper_version": VERSION, "project": plan["project"], "project_name": plan["project_name"], "run_root": plan["run_root"], "read_targets": plan["read_targets"], "publication_eligible_candidates": plan["publication_eligible_candidates"], "required_candidate_notes": plan["required_candidate_notes"], "recommended_candidate": plan["recommended_candidate"], "candidates": plan["candidates"], "review_method_required": "openclaw-read", "selection_policy": plan["selection_policy"], "assertive_override_instruction": plan["assertive_override_instruction"], "instruction": "Use OpenClaw Read on every read_targets.path exactly as returned. Choose the least aggressive amount that removes the unwanted green cast without introducing magenta/purple or suppressing faint structure. Then call select-publish.", "saturation_processing_permitted": False}
...
def self_test(timeout_seconds: int) -> dict[str, Any]:
    ...
    project_name = "Synthetic Green Reduction v1.0.1"
