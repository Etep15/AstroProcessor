...
description: "Run StarNet removal through the canonical v1.5.6 context-safe orchestrator, visually compare compact candidate panels, and publish native starless/starmask/unscreen products."
user-invocable: true
...
Installed orchestration version: **1.5.6**.
...
Successful v1.5.6 publication also writes:

```text
processing/starnet/orchestration-review-v1.5.6.json
```
...
├── orchestration-review-v1.5.6.json
...
characters. Semicolons inside an observation are supported by v1.5.6 and must
not be interpreted as unlabeled fields.



## v1.5.6 review enum contract

The following three structured fields are enums, not prose:
...
Descriptive details such as "none visible in the starless panel" or "no
significant halos" belong in the 80+ character `observation`, not in an enum
field.

The orchestrator recommendation is a tie-breaker only. CodeWarrior must first
record the actual visible candidate comparison. It must not justify selection
merely because a candidate is the recommended default.

Do not reuse the exact same observation text for every candidate.
