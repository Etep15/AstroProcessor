...
STATE_DIR_NAME = ".siril-black-point-v1.0.6"
LEGACY_STATE_DIR_NAME = ".siril-black-point-v1.0.5"
...
def legacy_call(
    ...
    *,
    ...
    authorized_obsolete_bridge: bool = False,
    ...
    env = os.environ.copy()
    # This private compatibility marker is owned exclusively by this orchestrator.
    # Never inherit it from the caller and never expose it as a public CLI flag.
    env.pop("BLACK_POINT_AUTHORIZED_OBSOLETE", None)
    if authorized_obsolete_bridge:
        env["BLACK_POINT_AUTHORIZED_OBSOLETE"] = "1"
    proc = subprocess.run(
...
def project_paths(project: str) -> dict[str, Path]:
    ...
    return {
        ...
        "legacy_intent": root / LEGACY_STATE_DIR_NAME / "fresh-intent.json",
    }
...
def matching_intent(project: str, snap: dict[str, Any]) -> tuple[Path, dict[str, Any]] | None:
    ...
    # Accept an exact hash-bound v1.0.5 authorization after upgrade so an
    # already-confirmed rerun is never forced to ask the user again.
    for path in (paths["intent"], paths["legacy_intent"]):
        if not path.is_file():
            continue
        try:
            obj = load_json(path)
            ...
            continue
        if intent_matches_snapshot(obj, snap):
            return path, obj
    return None
...
def command_advance(args: argparse.Namespace) -> int:
    ...
    if snap.get("canonical_exists"):
        ...
        rc, payload, _ = legacy_call(
            ...
            authorized_obsolete_bridge=(snap.get("canonical_status") == "obsolete"),
        )
        ...
        payload["authorized_obsolete_bridge_used"] = (
            snap.get("canonical_status") == "obsolete"
        )
        if rc != 0:
            payload.setdefault("action", "stop_no_discovery")
            payload.setdefault("directory_discovery_permitted", False)
            payload.setdefault("canonical_move_or_rename_permitted", False)
            payload.setdefault("manual_manifest_inspection_permitted", False)
        return emit(payload, rc)
...
def command_self_test(args: argparse.Namespace) -> int:
    return emit({
        ...
        "processing_helper_reports_compatibility_version": "1.0.4",
        "processing_math_unchanged": True,
        "authorized_obsolete_helper_bridge": True,
        "authorized_obsolete_bridge_private_environment": "BLACK_POINT_AUTHORIZED_OBSOLETE",
        "authorized_obsolete_bridge_exact_error_only": True,
        ...
        "legacy_v1_0_5_authorization_accepted_when_hash_binding_matches": True,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Black-point v1.0.6 fast completed/obsolete orchestration.")
