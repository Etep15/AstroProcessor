...
Orchestration version: **1.0.3**  
Processing engine version: **1.0.3**
...
The v1.0.3 processing engine generates three target-adaptive halo-neutralization candidates using Siril 1.4.4 only for image transforms.
...
3. in v1.0.3, holds the validated candidate-01 brightness compression constant across all three candidates so the review isolates halo neutralization;
4. is checked for clipping and preservation metrics;
5. is returned as an exact preview path for visual review.

Candidate family:

- candidate-00 — control: current thresholded global neutralization with validated balanced dimming
- candidate-01 — targeted halo cleanup: same dimming and same core neutralization, plus background-threshold-disabled desaturation only for magenta→pink, pink→orange, and orange→yellow
- candidate-02 — full-neutral upper bound: same dimming, but full-hue desaturation with the background threshold disabled
...
- whether faint blue/green stars are preserved where visually acceptable;
...
The goal is mostly neutral/white-looking prominent stars, with yellow/orange/magenta halos removed. Faint blue/green stars are acceptable and should not be treated as a defect by themselves. The validated bright-star dimming remains fixed across the three v1.0.3 candidates.
...
Notes must specifically address warm/magenta halo suppression, faint blue/green preservation, bright-star dominance, dim-star retention, profiles/artifacts, and overall balance.
