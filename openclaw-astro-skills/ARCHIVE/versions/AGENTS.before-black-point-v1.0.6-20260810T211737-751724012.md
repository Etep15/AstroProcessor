<!-- OPENCLAW_BLACK_POINT_FAST_ROUTE_V105_BEGIN -->
...
Do not read `astro-processing` or the black-point SKILL/source before that
first Exec. Do not discover project directories, helper source, Python,
run roots, or manifests with `ls`, `find`, `tree`, `grep`, `jq`, globbing, or
system Python.

If `confirmation_required` is returned, ask exactly the returned question.
After a clear affirmative response, run:
...
Then complete exact-path image review, autonomous selection, publication and
final verification without another normal user prompt.
<!-- OPENCLAW_BLACK_POINT_FAST_ROUTE_V105_END -->
