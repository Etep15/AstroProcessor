...
def review_plan(state:dict[str,Any],round_result:dict[str,Any])->dict[str,Any]:
    ...
    return {"status":"visual_review_required","review_scope":"round","orchestration_version":VERSION,
            ...
            "selection_rule":"Choose the technically eligible candidate with the best balance of brightness, contrast, rich retained color, structure, acceptable (possibly grey/lifted) background, and safe highlights. Use the numeric saturation/chroma metrics to ground the color judgment; do not call a candidate color-rich merely because it is more saturated than the immediately preceding source. Histogram locations and any M16 manual reference are guides only.",
            "minimum_rounds":MIN_ROUNDS,"maximum_rounds":MAX_ROUNDS,
            "instruction":"Read every returned target verbatim and compare every reviewable candidate. Reconcile visual color appearance with candidate_metrics. A dark/black background is not automatically better; a lifted grey background is acceptable when it preserves richer color and nebular structure. Round 1 must continue. From round 2 onward, stop only when another safe GHS+BP cycle is unlikely to materially improve brightness/contrast/color balance; do not stop merely because contrast is already strong."}
...
def final_panel(state:dict[str,Any])->dict[str,Any]:
    ...
    return {"status":"visual_review_required","review_scope":"final","orchestration_version":VERSION,"project_name":state['project_name'],
            ...
            "read_target_policy":{"path_handling":"verbatim","directory_discovery_forbidden":True,
              "forbidden_recovery_tools":["ls","find","grep","jq","globbing"],"on_read_failure":"stop_and_report_exact_failed_path"},
            "selection_rule":"Select the final candidate with the best overall balance of brightness, contrast, genuinely rich color, preserved structure, acceptable background and zero newly clipped data. Ground color claims in the numeric saturation/chroma metrics as well as the image. A later round does not automatically win, and a blacker background does not automatically win.",
            "instruction":"Read all three final targets verbatim, compare them, reconcile visual appearance with candidate_metrics and any advisory manual calibration reference, then call select-publish with the selected final candidate and substantive notes for all three."}
...
def command_select_round(a)->int:
    ...
    n=notes(a.note,expected); selected=next(c for c in reviewable if c['candidate']==a.candidate)
