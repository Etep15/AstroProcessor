...
CANDIDATES = [
    ...
    {
        "name": "candidate-01",
        "label": "validator-artifact-mask-cleanup",
    ...
    {
        "name": "candidate-02",
        "label": "strong-validator-artifact-mask-cleanup",
        "mode": "validator-artifact-mask",
        "threshold_quantile": 0.99,
        "knee_strength": 0.8,
        "base_scale": 0.92,
...
def build_validator_artifact_blend_mask(source_rgb: np.ndarray, control_rgb: np.ndarray, params: dict) -> tuple[np.ndarray, dict]:
    ...
    direct = artifact.astype(np.float32) * float(params["direct_strength"])
...
def residual_edge_metrics(source_rgb: np.ndarray, out_rgb: np.ndarray, edge_core_quantile: float = 0.997, inner_radius: int = 1, outer_radius: int = 7) -> dict:
    src_lum = source_rgb.mean(axis=-1)
    edge_core_threshold = float(np.quantile(src_lum, edge_core_quantile))
    core = (src_lum >= edge_core_threshold).astype(np.float32)
    outer = convolve_fft(core, np.ones((2*outer_radius+1, 2*outer_radius+1), dtype=np.float32), boundary='fill', fill_value=0.0, normalize_kernel=False) > 0
    inner = convolve_fft(core, np.ones((2*inner_radius+1, 2*inner_radius+1), dtype=np.float32), boundary='fill', fill_value=0.0, normalize_kernel=False) > 0
    ring = np.asarray(outer & ~inner)
    if not np.any(ring):
        ring = src_lum >= np.quantile(src_lum, 0.995)
...
def perform_candidate_generation(input_fit: Path, run_root: Path) -> dict:
    ...
    for candidate in CANDIDATES:
        ...
        # out_fit is already cdir/processed.fit; do not copy it onto itself.
        shutil.copy2(common / "control-preview.png", out_png) if candidate["mode"] == "control" else run_siril_script(cdir, build_preview_script())
...
def advance(project_name: str) -> dict:
    ...
    return {
        ...
        "selection_rule": "Select the candidate with the best overall balance of neutral star color, minimal red specks and purple fringe around bright stars, reduced star dominance, retention of dim blue/green stars, preserved profiles and no clipping.",
        ...
        "read_targets": [
            {"candidate": c["candidate"], "path": c["png_path"], "sha256": c["png_sha256"]}
            for c in summary["candidates"]
        ],
        "required_review_fields": ["color_neutrality","red_specks","purple_fringe","bright_star_dominance","dim_star_retention","profiles","overall_balance"],
    }
