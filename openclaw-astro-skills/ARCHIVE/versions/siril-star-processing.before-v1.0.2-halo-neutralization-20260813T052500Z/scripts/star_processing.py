...
def compare_metrics(src: np.ndarray, out: np.ndarray) -> dict:
    ...
    return {
        ...
        # This correlation is diagnostic only. Intentional nonlinear bright-star
        # compression necessarily lowers linear correlation even when spatial
        # star morphology is preserved.
...
def build_script(candidate: dict, threshold: float) -> str:
    ...
    # Continuous soft-knee compression: values at/below threshold receive only
    # the mild base scale; increasingly bright pixels receive progressively
    # stronger compression. This keeps dim stars present while reducing the
    # visual dominance of bright star cores.
    expr = (
    ...
    return "\n".join([
        ...
        "load input.fit",
        f'pm "{expr}" -nosum',
        # Neutralize last so the nonlinear brightness compression cannot
        # re-introduce channel imbalance into the final star layer.
        f"satu {candidate['desat']:.2f} {candidate['background_factor']:.2f} 6",
        "save processed.fit",
        "savepng preview",
        "close",
        "",
...
def advance(project_name: str) -> dict:
    ...
    return {
        ...
        "run_root": str(run_root),
        "selection_rule": "Select the candidate with the best overall balance of neutral star color, reduced star dominance, retention of dim stars, preserved profiles and no clipping.",
        "read_target_policy": {
...
def main(argv=None):
    ...
    if args.cmd == "select-publish":
        if len(args.compared) != len(args.note):
            raise SystemExit("--compared and --note must have the same count")
