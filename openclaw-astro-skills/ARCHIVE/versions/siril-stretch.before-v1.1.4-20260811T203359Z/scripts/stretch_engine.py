...
def metrics(path:Path,reference:Path|None=None,stride:int=2)->dict[str,Any]:
    a=rgb_array(path,stride=stride); y=luma(a); f=y.reshape(-1)
    ...
    out={
        "path":str(path),"sha256":sha256_file(path),"minimum":float(a.min()),"maximum":float(a.max()),
        "finite_fraction":1.0,"mode":histogram_mode(f),"median":qs["p50"],"saturation_median":robust_saturation(a),
        "low_clip_fraction":float(np.mean(a<=0.0)),"high_clip_fraction":float(np.mean(a>=1.0)),
        "channel_low_clip_fraction":[float(np.mean(a[c]<=0.0)) for c in range(3)],
        "channel_high_clip_fraction":[float(np.mean(a[c]>=1.0)) for c in range(3)],
        "channel_quantiles":channel_quantiles,
        "spread_p95_p05":qs["p95"]-qs["p5"],"spread_p99_p01":qs["p99"]-qs["p1"],**qs,
    }
    if reference is not None:
        b=rgb_array(reference,stride=stride); by=luma(b)
        ...
        # A monotonic non-linear stretch can have modest Pearson correlation while
        # still preserving spatial/luminance ordering extremely well.
        if x.size>120000:
        ...
        out["low_clip_delta"]=out["low_clip_fraction"]-float(np.mean(b<=0.0))
        out["high_clip_delta"]=out["high_clip_fraction"]-float(np.mean(b>=1.0))
    return out
...
def ghs_safety_reasons(m:dict[str,Any])->list[str]:
    reasons=[]
    if m.get('high_clip_delta',0.0)>1e-7 or m['high_clip_fraction']>1e-7:
        reasons.append('GHS introduced highlight clipping')
    # Quantile headroom is more robust than rejecting a valid image merely because
    # one isolated pixel is bright. Exact clipping is still forbidden above.
    if m['p99.9']>0.985:
...
def bp_safety_reasons(m:dict[str,Any])->list[str]:
    reasons=[]
    if m.get('low_clip_delta',0.0)>0.0005: reasons.append('meaningful shadow clipping increased')
    if m.get('high_clip_delta',0.0)>1e-7 or m['high_clip_fraction']>1e-7: reasons.append('highlight clipping')
...
def execute_candidate(source:Path,round_dir:Path,round_number:int,index:int,params:GHSParameters,timeout:int)->dict[str,Any]:
    ...
    for n,factor in enumerate(ghs_backoff_factors()):
        ...
        ghs_attempts.append({"factor":factor,"params":trial_params.as_dict(),"command":trial_params.command(),
                             ...
                             "metrics":{k:mm.get(k) for k in ('minimum','maximum','mode','p0.1','p1','p50','p95','p99','p99.9','low_clip_delta','high_clip_delta','luma_correlation','luma_rank_correlation','saturation_retention')}})
        if not safety:
    ...
    for n,factor in enumerate(backoff_factors()):
        ...
        attempts.append({"factor":factor,"bp":bp,"safe":not safety,"safety_reasons":safety,
                         "metrics":{k:mm.get(k) for k in ('minimum','maximum','p0.1','p1','p50','p95','p99','p99.9','low_clip_delta','high_clip_delta','luma_correlation','luma_rank_correlation','saturation_retention')}})
        if not safety:
...
def rejection_summary(candidates:list[dict[str,Any]])->list[dict[str,Any]]:
    ...
    for c in candidates:
        ...
        out.append({"candidate":c['candidate'],"eligible":c['eligible'],"rejection_reasons":c['rejection_reasons'],
                    ...
                    "metrics":{k:m.get(k) for k in ('minimum','maximum','mode','p0.1','p1','p5','p50','p95','p99','p99.9','spread_p95_p05','spread_p99_p01','low_clip_fraction','high_clip_fraction','low_clip_delta','high_clip_delta','luma_correlation','luma_rank_correlation','saturation_median','saturation_retention')},
