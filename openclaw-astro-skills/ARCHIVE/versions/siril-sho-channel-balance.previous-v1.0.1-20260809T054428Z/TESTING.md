# siril-sho-channel-balance v1.0.1 testing
...
## v1.0.1 routing regression

Before the real M16 production test, verify:

- the stage skill explicitly identifies the canonical CodeWarrior Projects root;
- the stage skill forbids AstroProcessor, ASIAIR discovery and alternate roots;
- the broad `astro-processing` skill contains the stage-request router;
- the router explicitly maps `SHO channel balance` to
  `siril-sho-channel-balance`;
- the broad router forbids project initialization/import/prepare before
  handing off a named stage request;
- M16 `advance --plan-only` resolves the project beneath
  `/home/peter/.openclaw/workspace/agents/codewarrior/Projects`;
- the validated pure SHO FITS and manifest remain byte-for-byte unchanged.
