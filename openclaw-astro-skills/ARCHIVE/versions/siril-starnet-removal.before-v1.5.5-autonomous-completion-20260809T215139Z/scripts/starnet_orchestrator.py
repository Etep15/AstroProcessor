...
STATE_DIR_NAME = ".siril-starnet-v1.5.4"
...
def paths_for(project_name: str) -> dict[str, Path]:
    ...
    return {
        ...
        "orchestration_review": canonical / "orchestration-review-v1.5.4.json",
        "state": state,
...
def build_state(project_name: str, run: dict[str, Any], intent: Path) -> dict[str, Any]:
    ...
    contact_dir = run_root / "compact-review" / "v1.5.4-panels"
...
def review_publish(project_name: str, candidate: str, selection_rationale: str, note_values: list[str]) -> dict[str, Any]:
    ...
    completed_review = Path(state["run_root"]) / "compact-review" / "visual-review-v1.5.4-completed.json"
...
def build_parser():
    ap = argparse.ArgumentParser(description="StarNet v1.5.4 context-safe orchestration wrapper.")
