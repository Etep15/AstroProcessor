...
COMPATIBLE_RUN_HELPER_VERSIONS = frozenset({"1.0.1", "1.0.2", "1.0.3"})
...
def parse_candidate_notes(values: list[str], eligible: list[str]) -> dict[str, str]:
    ...
    for raw in values:
        if "=" not in raw:
            raise GreenReductionError(
                "Each --note must use candidate-NN=green:<...>; magenta:<...>; structure:<...>."
            )
        ...
        if candidate not in expected:
            raise GreenReductionError(
                f"Candidate visual note references {candidate!r}; eligible candidates are {sorted(expected)}."
            )
        if candidate in notes:
            raise GreenReductionError(f"Duplicate candidate visual note for {candidate}.")

        fields: dict[str, str] = {}
        for segment in [part.strip() for part in body.split(";") if part.strip()]:
            if ":" not in segment:
                raise GreenReductionError(
                    f"{candidate} note must contain labeled green:, magenta:, and structure: fields."
                )
            key, value = segment.split(":", 1)
            key = key.strip().lower()
            value = value.strip()
            if key in fields:
                raise GreenReductionError(f"{candidate} note repeats the {key!r} field.")
            fields[key] = value

        required_fields = {"green", "magenta", "structure"}
        if set(fields) != required_fields:
            raise GreenReductionError(
                f"{candidate} note must contain exactly green:, magenta:, and structure:. "
                f"Got {sorted(fields)}."
            )
        ...
        green_text = fields["green"].lower()
        if not any(token in green_text for token in (
            "green", "cast", "residual", "remain", "removed", "reduce", "balanced", "unwanted"
        )):
            raise GreenReductionError(
                f"{candidate} green: field must describe residual/unwanted green or its removal."
            )

        magenta_text = fields["magenta"].lower()
        if not any(token in magenta_text for token in (
            "magenta", "purple", "shift", "none", "no ", "not ", "visible", "over-correction", "overcorrection"
        )):
            raise GreenReductionError(
                f"{candidate} magenta: field must state whether magenta/purple over-correction is visible."
            )

        structure_text = fields["structure"].lower()
        if not any(token in structure_text for token in (
            "faint", "pillar", "dark", "lane", "structure", "emission", "preserv", "nebula"
        )):
            raise GreenReductionError(
                f"{candidate} structure: field must describe faint/Pillars/dark-lane preservation."
            )
    ...
    if actual != expected:
        raise GreenReductionError(
            f"Candidate visual notes must cover every eligible candidate exactly. "
...
def compact_review_plan(plan: dict[str, Any]) -> dict[str, Any]:
    payload = {
        ...
        "action": "read_exact_targets_then_select_publish",
        ...
        "candidate_note_format": (
            "candidate-NN=green:<specific observation>; "
            "magenta:<specific magenta/purple observation>; "
            "structure:<specific faint/Pillars/dark-lane observation>"
        ),
        "candidate_note_requirements": [
            "green: say what residual green/unwanted green remains or was removed",
            "magenta: say whether magenta/purple over-correction is visible",
            "structure: say whether faint emission/Pillars/dark-lane structure is preserved",
        ],
        "selection_rule": (
            "Choose the least aggressive eligible candidate that removes unwanted green "
            "without magenta/purple or loss of faint structure."
        ),
        "candidate_02_override_required_if_selected": True,
        "instruction": (
            "Read every read_targets[].path verbatim. If any Read fails, STOP; do not use ls/find "
            "or inspect the run directory. Review residual green, magenta/purple, and faint structure; "
            "then select-publish using the exact three-field note format."
        ),
        "saturation_processing_permitted": False,
    }
    if len(json.dumps(payload).encode("utf-8")) > 4096:
        raise GreenReductionError("Visual-review handoff exceeded the 4096-byte compact-review limit.")
