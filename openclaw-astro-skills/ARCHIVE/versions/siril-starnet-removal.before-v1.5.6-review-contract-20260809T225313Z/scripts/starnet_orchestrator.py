...
STATE_DIR_NAME = ".siril-starnet-v1.5.5"
...
def paths_for(project_name: str) -> dict[str, Path]:
    ...
    return {
        ...
        "orchestration_review": canonical / "orchestration-review-v1.5.5.json",
        "state": state,
...
def build_state(project_name: str, run: dict[str, Any], intent: Path) -> dict[str, Any]:
    ...
    contact_dir = run_root / "compact-review" / "v1.5.5-panels"
...
def review_plan(state: dict[str, Any]) -> dict[str, Any]:
    ...
    for name in state["generated_candidates"]:
        ...
        note_examples.append(
            f'{name}=accepted:{accepted}; remaining_stars:none visible in the '
            'starless panel; broad_nebula:false; nebula_damage:no obvious '
            'nebular knots, filaments, or dark-lane structure are removed; '
            'halos:no significant halos or seams are visible; observation:'
            'Compare the starless image, both starmask views, and the unscreen '
            'layer against the other candidates and record specific visible '
            'differences while preserving M16 structure.'
        )
    ...
    return {
        ...
        "note_format": (
            ...
            "broad_nebula:<true|false>; nebula_damage:<specific>; "
            "halos:<specific>; observation:<at least 80 characters of "
            "candidate-specific visual comparison>"
        ),
        "note_parser": {
        ...
        "review_publish_command_template": {
            ...
            "required": [
                ...
                '--selection-rationale "<specific overall comparison; at least 80 characters>"',
...
def parse_notes(values: list[str], expected_candidates: list[str]):
    ...
    for raw in values:
        if "=" not in raw:
            raise StarNetOrchestrationError(
                "Each --note must start with candidate-NN=..."
            )
        name, body = raw.split("=", 1)
        name = name.strip()
        if name not in expected:
            raise StarNetOrchestrationError(
                f"Unexpected candidate in --note: {name!r}"
            )
        ...
        for piece in [x.strip() for x in field_boundary.split(body) if x.strip()]:
            if ":" not in piece:
                raise StarNetOrchestrationError(
                    f"{name} has an unlabeled review field."
                )
            key, value = piece.split(":", 1)
            key = key.strip().lower()
            if key not in required:
                raise StarNetOrchestrationError(
                    f"{name} contains unknown review field {key!r}."
                )
            if key in fields:
                raise StarNetOrchestrationError(
                    f"{name} repeats review field {key!r}."
                )
            fields[key] = " ".join(value.split())

        if set(fields) != required:
            raise StarNetOrchestrationError(
                f"{name} must contain exactly {list(ordered_keys)}; "
                f"got {sorted(fields)}"
            )
        ...
        if accepted_text not in {"true", "false"}:
            raise StarNetOrchestrationError(
                f"{name} accepted must be true or false."
            )
        if broad_text not in {"true", "false"}:
            raise StarNetOrchestrationError(
                f"{name} broad_nebula must be true or false."
            )
...
def review_publish(project_name: str, candidate: str, selection_rationale: str, note_values: list[str]) -> dict[str, Any]:
    ...
    completed_review = Path(state["run_root"]) / "compact-review" / "visual-review-v1.5.5-completed.json"
...
def build_parser():
    ap = argparse.ArgumentParser(description="StarNet v1.5.5 autonomous-completion orchestration wrapper.")
