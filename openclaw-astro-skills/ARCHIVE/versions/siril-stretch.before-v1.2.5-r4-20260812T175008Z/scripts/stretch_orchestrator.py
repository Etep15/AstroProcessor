...
M16_MANUAL_REFERENCE={
    ...
    "history_summary":"5 GHS applications with three BP shifts (0.14, 0.10, 0.02); broad locality approximately -0.43..+0.59",
...
def continuation_advisory(state:dict[str,Any],round_result:dict[str,Any],reviewable:list[dict[str,Any]])->dict[str,Any]:
    ...
    out={
      ...
      "median":m.get('p50'),"p95":m.get('p95'),"p99_9":m.get('p99.9'),
      ...
      "guidance":"From round 2 onward, protect bright structures explicitly. Stop when another cycle would mainly raise p95/p99.9 or local bright nebulosity without a material gain in visible colour separation or structure."
    }
    if state.get('source_sha256')==M16_SOURCE_SHA:
      out["m16_calibration_hint"]={
        ...
        "strong_stop_neighborhood":"For this M16 source, a reviewed candidate around median 0.18-0.23, p95 roughly <=0.34, p99.9 <=0.70, saturation >=0.26 and color_contrast_index >=0.31 is a strong reason to stop. Prefer highlight protection over more luminance when a bright pillar or edge is already too strong."
      }
    return out
...
def review_plan(state:dict[str,Any],round_result:dict[str,Any])->dict[str,Any]:
    ...
    return {"status":"visual_review_required","review_scope":"round","orchestration_version":VERSION,
            ...
            "instruction":"Read every returned target verbatim and compare every reviewable candidate. Reconcile visual appearance with saturation, chroma, color_contrast_index, median, p95 and p99.9. Pay particular attention to localized bright nebulosity/pillar edges that can be over-stretched even when the median looks good. A lifted background is acceptable, but washed/pastel color from excess luminance is not. Round 1 must continue. From round 2 onward, stop when another cycle would mainly raise luminance/upper-tail placement without a material gain in color separation or structure; do not chase brightness for its own sake."}
...
def final_review_plan(state:dict[str,Any])->dict[str,Any]:
    ...
    return {"status":"visual_review_required","review_scope":"final","orchestration_version":VERSION,"project_name":state['project_name'],
            ...
            "selection_rule":"Select the final candidate with the best overall balance of brightness, contrast, genuinely separated color, preserved structure, acceptable background and zero newly clipped data. Ground color claims in saturation, chroma and color_contrast_index. Prefer a slightly less-stretched or highlight-protected candidate when bright nebulosity or pillar edges are pushed too hard, even if the global median is acceptable. A later round does not automatically win.",
