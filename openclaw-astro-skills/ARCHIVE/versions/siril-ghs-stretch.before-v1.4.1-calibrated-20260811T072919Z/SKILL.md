...
## v1.3.3 resumable autonomous publication contract — highest priority

Version 1.3.3 is an orchestration/publication patch only. The GHS processing
engine remains **1.3.1** byte-for-byte unchanged.

This skill remains fully standalone. A future pipeline controller may invoke it,
but no external pipeline orchestrator is required for this stage to complete.

### Resume an already-generated run

If a v1.3.2 active run is already waiting for visual selection, `advance` must
resume that exact run. It must not regenerate candidates and must not require a
second fresh-run confirmation.

The v1.3.3 wrapper intentionally retains the existing v1.3.2 state directory so
the already-expensive candidate run can be migrated in place.
...
OpenClaw Read every returned `read_targets[].path` exactly as returned. Do not
use `ls`, `find`, `cat`, `grep`, `jq`, globbing or source-code discovery.

After reviewing every eligible candidate, invoke the exact returned
`select_publish_command_template`.

Candidate-specific structured reviews belong in repeated `--note` arguments:

```text
--note "candidate-01=stretch:<...>; structure:<...>; color:<...>; noise:<...>; highlights:<...>"
--note "candidate-02=stretch:<...>; structure:<...>; color:<...>; noise:<...>; highlights:<...>"
```

Do **not** place candidate records inside `--visual-notes`.

`--visual-notes` is optional in v1.3.3. If omitted, the orchestrator derives the
overall comparison from the validated candidate notes.

The normal autonomous publication therefore requires only:

```text
.../bin/ghs-stretch select-publish \
  --project "<project>" \
...
### Bounded publication repair

Publication review-format repairs are limited to **3 attempts**. A format error
must never cause:

```text
GHS candidate regeneration
image rereads
directory discovery
...
If the third publication-format attempt fails, stop and report the exact contract
error.

### Pass-2 compatibility

The installed GHS pass-2 gate already requires:

```text
processing/ghs-pass1/visual-selection-record-v1.3.2.json
```

Therefore v1.3.3 deliberately keeps that compatibility filename and stores
`orchestration_version: 1.3.2` in the gate-facing record while also recording:

```text
publication_orchestration_version: 1.3.3
review_contract_revision: ghs-pass1-publication-v1.3.3
```

This lets the standalone pass-1 publication fix remain compatible with the
already-installed standalone pass-2 gate.


## v1.4.0 dynamic-range expansion policy — highest priority

This section supersedes older pass-1 policy text below wherever it conflicts.

Installed standalone orchestration: **1.4.0**. Processing engine: **1.4.0**.
Policy revision: **dynamic-range-expansion-v1**.

The pass-1 objective is no longer to drive the image toward a narrow median target
or preserve the almost-linear source percentile shape. Pass 1 must create useful,
permanent signal separation while remaining clearly intermediate to pass 2.

The bounded candidate policy remains a maximum of **3** candidates. For each
candidate the engine records median, p90, p99, p99/median, p99-minus-median,
clipping, maximum, luminance correlation and inverse-GHT roundtrip evidence.

Publication eligibility requires all ordinary technical safeguards plus:

```text
broad pass-1 median window: 0.080–0.240
minimum p99/median:         1.20
preferred p99/median:       >= 1.25
minimum p99 - median:       0.025
maximum p99:                0.780
maximum pixel:              0.950
preferred luma correlation: >= 0.970
clipping:                    none
```

Median 0.150 is an advisory center only. A candidate near 0.150 that remains
histogram-compressed is **too_gentle** and is not publication eligible.

Source-relative percentile-shape preservation is explicitly **false**. Candidate
planning uses source p95/p99 measurements to place SP in useful signal above the
background peak and uses moderate B values to broaden the affected range. The
numerical recommendation weights dynamic-range separation more strongly than
median proximity. Visual review remains authoritative among eligible candidates.

The source remains only:

```text
processing/sho-channel-balance/SHO-starless-linear-balanced.fit
```

All existing source/provenance, RGB Blend, even-weighted luminance, inverse-GHT,
32-bit RGB, clipping, preview and publication safeguards remain in force.

v1.4.0 uses a new orchestration state directory:

```text
.siril-ghs-stretch-v1.4.0/
```

Candidate runs from v1.3.3 or earlier are preserved but are never resumed under
this changed processing policy. A completed v1.3.1 canonical is treated as
**completed-but-policy-obsolete** and requires one explicit fresh-rerun
confirmation. Before confirmation, completed-stage detection is manifest-first
and must not hash the large FITS files. Confirmation performs strong SHA-256
binding to both current source/canonical FITS and their manifests.

The exact OpenClaw Read-target, structured candidate-note, autonomous selection,
preservation-safe publication, and `visual-selection-record-v1.3.2.json`
compatibility filename contracts are retained.

This stage still stops after GHS pass 1. The currently installed GHS pass-2 policy
was calibrated against the old pass-1 result and must be recalibrated after the
new v1.4.0 pass-1 result is reviewed and published.
