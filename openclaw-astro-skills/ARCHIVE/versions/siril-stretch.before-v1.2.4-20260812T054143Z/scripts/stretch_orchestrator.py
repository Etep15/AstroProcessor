...
def active_version(state:dict[str,Any])->str:
    ...
    if v not in {"1.2.1", VERSION}:
        raise OrchestrationError(f"Active stretch run version {v!r} is not resume-compatible with orchestration {VERSION}; preserve it and start a fresh run only after explicit recovery/authorization.")
    return v
