...
def publish_project(
    ...
    record["source_contract_revision"] = SOURCE_CONTRACT_REVISION
    record["next_stage"] = NEXT_STAGE
    record["sho_channel_balance_permitted"] = True
    record["ghs_pass1_permitted"] = False
...
def status_project(
    ...
    if (
        ...
        return {
            ...
            "reason": (
                "Existing StarNet output predates the native StarNet to "
                "SHO channel-balance source contract."
            ),
            "project": str(paths["project"]),
            "stable_directory": str(paths["stable"]),
            "manifest": str(paths["stable_manifest"]),
    ...
    try:
        ...
        if manifest.get("sho_channel_balance_permitted") is not True:
            errors.append("Manifest does not permit SHO channel balance.")
        if manifest.get("ghs_pass1_permitted") is not False:
            errors.append("StarNet manifest incorrectly permits GHS pass 1 directly.")
        if manifest.get("next_stage") != NEXT_STAGE:
            errors.append("StarNet manifest next stage is not SHO channel balance.")
        if manifest.get("starless_processing_permitted") is not True:
            errors.append("StarNet manifest does not permit starless processing.")
        if manifest.get(
    ...
    ready = (
        ...
        and manifest.get("source_contract_revision") == SOURCE_CONTRACT_REVISION
        and manifest.get("sho_channel_balance_permitted") is True
        and manifest.get("ghs_pass1_permitted") is False
    )
    return {
        ...
        "sho_channel_balance_permitted": ready,
        "ghs_pass1_permitted": False,
        "starless_background_processing_permitted": False,
...
def self_test(workspace: Path, timeout_seconds: int) -> dict[str, Any]:
    ...
    if (
        ...
        or checked.get("sho_channel_balance_permitted") is not True
        or checked.get("ghs_pass1_permitted") is not False
        or checked.get(
            "starless_background_processing_permitted"
        ) is not False
    ...
    return {
        ...
        "sho_channel_balance_permitted": checked["sho_channel_balance_permitted"],
        ...
        "tests": [
            ...
            "SHO channel-balance downstream permission",
