...
STATE_DIR_NAME = ".siril-starnet-v1.5.3"
...
def paths_for(project_name: str) -> dict[str, Path]:
    ...
    return {
        ...
        "orchestration_review": canonical / "orchestration-review-v1.5.3.json",
        "state": state,
...
def confirm_fresh(project_name: str) -> dict[str, Any]:
    p = validate_project(project_name)
    snap = canonical_snapshot(project_name)
...
def build_state(project_name: str, run: dict[str, Any], intent: Path) -> dict[str, Any]:
    ...
    contact_dir = run_root / "compact-review" / "v1.5.3-panels"
...
def review_plan(state: dict[str, Any]) -> dict[str, Any]:
    ...
    note_examples = [
        f'{name}=accepted:{"true" if name == state.get("recommended_candidate") else "false"}; '
        'remaining_stars:none; broad_nebula:false; nebula_damage:none; halos:none; '
        'observation:<specific comparison from the contact panel>'
        for name in state["generated_candidates"]
    ]
    return {
        "status": "visual_review_required",
        "action": "read_exact_source_and_candidate_panels_then_review_publish",
        "orchestration_version": VERSION,
        "processing_engine_version": ENGINE_VERSION,
        ...
        "note_format": (
            "candidate-NN=accepted:<true|false>; remaining_stars:<specific>; "
            "broad_nebula:<true|false>; nebula_damage:<specific>; halos:<specific>; "
            "observation:<specific visual comparison>"
        ),
        "review_publish_command_template": {
            ...
            "required": [
                ...
                '--selection-rationale "<overall comparison>"',
...
def advance(project_name: str) -> dict[str, Any]:
    ...
    if snap.get("exists"):
        ...
        if intent is None:
            return {
                ...
                "current_canonical_status": snap.get("status"),
                "current_canonical_starless_sha256": snap.get("starless_sha256"),
                "canonical_output_preserved": True,
            }
    ...
    # Create a synthetic consumed authorization marker only for state bookkeeping.
    p["intents"].mkdir(parents=True, exist_ok=True)
...
def parse_notes(values: list[str], expected_candidates: list[str]):
    ...
    result = {}
    required = {"accepted", "remaining_stars", "broad_nebula", "nebula_damage", "halos", "observation"}
    for raw in values:
        if "=" not in raw:
        ...
        if set(fields) != required:
            raise StarNetOrchestrationError(
                f"{name} must contain exactly {sorted(required)}; got {sorted(fields)}"
            )
        ...
        if len(fields["observation"]) < 30:
            raise StarNetOrchestrationError(f"{name} observation is too vague.")
        for key in ("remaining_stars", "nebula_damage", "halos"):
            if len(fields[key]) < 3:
                raise StarNetOrchestrationError(f"{name} {key} is too vague.")
        result[name] = {
    ...
    if set(result) != expected:
        raise StarNetOrchestrationError(
            f"Review notes must cover every generated candidate exactly. "
...
def review_publish(project_name: str, candidate: str, selection_rationale: str, note_values: list[str]) -> dict[str, Any]:
    ...
    if len(" ".join(selection_rationale.split())) < 40:
        raise StarNetOrchestrationError("Selection rationale is too vague.")
    ...
    completed_review = Path(state["run_root"]) / "compact-review" / "visual-review-v1.5.3-completed.json"
...
def stage_status(project_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "canonical_starless_sha256": snap.get("starless_sha256"),
        "source_contract_revision": SOURCE_CONTRACT_REVISION,
...
def self_test() -> dict[str, Any]:
    ...
    notes = parse_notes([
        ...
        "nebula_damage:none visible; halos:none visible; "
        "observation:Starless structure is preserved and the mask contains localized stellar signal.",
        "candidate-01=accepted:false; remaining_stars:minor residuals; broad_nebula:false; "
        "nebula_damage:none visible; halos:none visible; "
        "observation:Several faint stellar residuals remain compared with candidate-00.",
...
def build_parser():
    ap = argparse.ArgumentParser(description="StarNet v1.5.3 context-safe orchestration wrapper.")
