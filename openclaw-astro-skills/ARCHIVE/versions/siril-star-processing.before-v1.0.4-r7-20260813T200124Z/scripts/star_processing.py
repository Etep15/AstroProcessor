...
CANDIDATES = [
    ...
    {
        "name": "candidate-01",
        "label": "strong-purple-fringe-cleanup",
        "mode": "validator-artifact-mask",
        "threshold_quantile": 0.99,
        "knee_strength": 0.8,
        "base_scale": 0.92,
    ...
    {
        "name": "candidate-02",
        "label": "full-purple-fringe-cleanup",
        "mode": "validator-artifact-mask",
        "threshold_quantile": 0.99,
        "knee_strength": 0.8,
        "base_scale": 0.92,
...
def perform_candidate_generation(input_fit: Path, run_root: Path) -> dict:
    ...
    for candidate in CANDIDATES:
        ...
        # pixels detected in the accepted r5 control within the 2-7 px edge ring.
        ...
        outer = convolve_fft(core, np.ones((2*outer_r+1, 2*outer_r+1), dtype=np.float32), boundary='fill', fill_value=0.0, normalize_kernel=False) > 0
        inner = convolve_fft(core, np.ones((2*inner_r+1, 2*inner_r+1), dtype=np.float32), boundary='fill', fill_value=0.0, normalize_kernel=False) > 0
        ring = np.asarray(outer & ~inner)
