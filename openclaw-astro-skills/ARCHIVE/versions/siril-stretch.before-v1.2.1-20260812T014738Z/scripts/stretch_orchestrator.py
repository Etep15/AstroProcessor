...
def active_version(state:dict[str,Any])->str:
    ...
    if v not in (VERSION,'1.1.4'):
        raise OrchestrationError(f"Active stretch run version {v!r} is not resume-compatible with {VERSION}.")
    return v
...
def review_plan(state:dict[str,Any],round_result:dict[str,Any])->dict[str,Any]:
    ...
    return {"status":"visual_review_required","review_scope":"round","orchestration_version":VERSION,
            "active_run_orchestration_version":state.get('orchestration_version'),
            "resume_compatible_from":"1.1.4" if state.get('orchestration_version')=='1.1.4' else None,
            ...
            "instruction":"Read every returned read_targets[].path verbatim and compare every candidate in compared_candidate_order. Three candidates are attempted each round, but technically rejected candidates may intentionally have no preview and must not be read or included in --compared/--note. Repeat --compared and --note once per returned candidate. Round 1 must continue; from round 2 onward continue only if another GHS+BP round is likely to improve the image safely."}
