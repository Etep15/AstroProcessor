...
Orchestration version: **1.1.5**  
Processing engine version: **1.1.4**
...
4. When `visual_review_required` with `review_scope: round`:
   ...
   - Compare every candidate returned in `read_targets` / `compared_candidate_order` autonomously. Three processing candidates are attempted each round, but a technically rejected candidate may intentionally have no preview and must not be visually reviewed.
   ...
   - Call `select-round` using the exact run root and `compared_candidate_order`. **Repeat** `--compared` once for each returned candidate in exact order, and repeat `--note` once for each returned candidate using `candidate-ID=brightness:...; contrast:...; color:...; structure:...; background:...; highlights:...`. Do not combine candidate IDs into one comma-separated `--compared` value.
...
5. If `technically_rejected_candidates` is non-empty, preserve that rejection evidence but do not attempt to read a missing preview or include that candidate in `--compared`/`--note`. Repeat review/selection autonomously until the runtime returns `review_scope: final`.
...
- BP is proposed from the actual post-GHS histogram, then automatically backed off and re-tested in Siril until there are zero newly clipped RGB pixels at full resolution and headroom gates are safe; a failed BP proposal never forces publication.
