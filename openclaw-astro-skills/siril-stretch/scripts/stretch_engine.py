...
ROUND2_PROFILES = [{'B': 0.0, 'bp_min_fraction': 0.985, 'color_mode': 'independent', 'd_scale': 0.96, 'hp_fraction': 0.74, 'lp_fraction': 0.0, 'name': 'highlight-protected', 'sp_scale': 1.27}, {'B': 0.0, 'bp_min_fraction': 0.985, 'color_mode': 'independent', 'd_scale': 1.0, 'hp_fraction': 1.0, 'lp_fraction': 0.0, 'name': 'balanced', 'sp_scale': 1.32}, {'B': 0.0, 'bp_min_fraction': 0.985, 'color_mode': 'independent', 'd_scale': 0.96, 'hp_fraction': 0.8, 'lp_fraction': 0.0, 'name': 'color-contrast', 'sp_scale': 1.27}]
# END_ROUND2_PROFILES
...
def round2_parameters(source_metrics:dict[str,Any],profile:dict[str,Any])->GHSParameters:
    ...
    hf=max(0.0,min(1.0,float(profile.get('hp_fraction',1.0))))
    hp=max(sp,min(1.0,sp+(1.0-sp)*hf))
    return bounded(base_D*float(profile['d_scale']),float(profile.get('B',0.0)),sp,lp,hp,str(profile.get('color_mode','independent')))
...
def score_candidate(m:dict[str,Any],source_metrics:dict[str,Any])->float:
    # v1.2.5-r4: preserve r3 transform math, but avoid choosing a globally dimmer
    # candidate as a substitute for genuine highlight protection. The penalties
    # are intentionally small and activate only outside the useful midtone/tail
    # neighborhood; visual review remains authoritative.
    ...
    peak_score=_band_score(m['mode'],0.19,0.18)
    median_score=_band_score(m['p50'],0.20,0.16)
    upper_score=_band_score(m['p99'],0.46,0.28)
    p95_score=_band_score(m['p95'],0.31,0.20)
    tail_score=_band_score(m['p99.9'],0.64,0.24)
    highlight=max(0.0,min(1.0,(0.90-m['p99.9'])/0.28))
    ...
    penalty=(max(0.0,m['p50']-0.245)*5.0 + max(0.0,m['mode']-0.245)*3.0 +
             max(0.0,m['p95']-0.34)*4.0 + max(0.0,m['p99.9']-0.70)*7.0 +
             max(0.0,m['p99']-0.60)*2.5 + max(0.0,m['maximum']-0.99)*8.0)
    # r4 tie-break correction: discourage excessive whole-image dimming and
    # upper-tail overshoot when a nearby candidate protects highlights while
    # retaining comparable saturation/chroma. For the calibrated M16 family,
    # this separates the intended highlight-protected profile from the darker
    # HP=1.0 alternative without changing any Siril transform parameters.
    under_midtone_penalty=max(0.0,0.180-float(m['p50']))*14.0
    tail_over_penalty=max(0.0,float(m['p99.9'])-0.700)*10.0
    return float(0.20*gain_score + 0.40*peak_score + 0.55*median_score + 0.34*upper_score +
                 0.46*p95_score + 0.62*tail_score + 0.48*highlight + 0.44*sat_score +
                 0.50*chroma_score + 0.82*contrast_score + 0.08*retention - penalty -
                 under_midtone_penalty - tail_over_penalty)
